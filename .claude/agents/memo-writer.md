...
```json
{
  ...
  "target_price": "<목표가>",
  "stop_loss": "<초기 손절가>",
  "position_sizing": "<권고 비중 X% / 최대 비중 Y% / 1회 진입 Z% / 진입가 2-Case: (A)현재가 즉시 진입 비중 + (B)눌림목 대기 진입 비중 / 추가 진입 조건>",
...
- **신규 투자자 (decision_new)**: 매수 / 조건부 매수 / **추세 추종 진입 / 돌파 시 추격매수** / 회피 + 구체적 행동
  ...
  - **진입가 2-Case 필수**: 눌림목에만 앵커링하지 말고 **(A) 현재가 즉시 진입**과 **(B) 눌림목 대기 진입**을 모두 제시하고 각각 진입 비중을 권고. 추세 유효 시 Case A(현재가 선진입)가 디폴트.
  - 예: "추세 추종 진입. (A)현재가에 9% 즉시 진입, (B)눌림목(50일선) 도달 시 6% 추가, 돌파 시 나머지 추격매수."
