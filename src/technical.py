...
def analyze_technical(ticker_symbol: str, period: str = "6mo") -> dict:
    ...
    vol_10d = _safe_round(float(volume_series.tail(10).mean()))
    vol_50d = _safe_round(float(volume_series.tail(50).mean())) if len(volume_series) >= 50 else None
    vol_ratio = _safe_round(float(volume_series.iloc[-1]) / vol_10d) if vol_10d and vol_10d > 0 else None
    ...
    if price_change_5d is not None and obv_trend != "N/A":
        if price_change_5d > 0 and obv_trend == "상승":
            volume_confirmation = "정합 (가격 상승 + 거래량 증가 = 강세 확인)"
        elif price_change_5d < 0 and obv_trend == "하락":
            volume_confirmation = "정합 (가격 하락 + 거래량 감소 = 약세 확인)"
        elif price_change_5d > 0 and obv_trend == "하락":
            volume_confirmation = "괴리 (가격 상승 + 거래량 감소 = 상승 약화 경고)"
        else:
            volume_confirmation = "괴리 (가격 하락 + 거래량 증가 = 매도 압력)"
    else:
    ...
    # --- 피보나치 되돌림 (fibonacci) ---
    high_52w = float(high.max()) if len(high) >= 200 else float(high.max())
    low_52w = float(low.min()) if len(low) >= 200 else float(low.min())
    fib_range = high_52w - low_52w
    fib_levels = {}
    if fib_range > 0:
        for level in [0.236, 0.382, 0.5, 0.618, 0.786]:
            fib_levels[f"{level*100:.1f}%"] = _safe_round(high_52w - fib_range * level)
    ...
    try:
        ...
        if not weekly_df.empty and len(weekly_df) >= 14:
            ...
            w_macd_hist = _safe_round(float(w_macd.macd_diff().iloc[-1]))
            ...
            weekly_signals = {
                ...
                "macd_signal": "매수" if w_macd_hist and w_macd_hist > 0 else "매도" if w_macd_hist and w_macd_hist < 0 else "중립",
                "sma_20": w_sma_20,
