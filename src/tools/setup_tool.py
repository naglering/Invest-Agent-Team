...
평가금액·손익·비중·손절/목표 판정을 계산해 줍니다.
매수/매도는 `portfolio add` / `portfolio sell` 권장 — 체결이 `data/trades.jsonl` 원장에
자동 기록되어 승률·평균 R(성과 피드백 루프)의 원천 데이터가 됩니다.

## 입력 규칙

- **종목**: 티커 (미국 `NVDA`, 한국 `005930.KS`)
- **수량**: 보유 주식 수
- **평단**: 1주당 평균 매입 단가 (해당 종목의 통화 기준)
- **통화**: `USD` 또는 `KRW` 권장 (그 외 EUR 등은 평가 시 `<CCY>KRW=X` 환율 자동 조회)
- **매입환율**: 비KRW 종목만 입력 (매입 시점 원화 환율, 예 `1380`). KRW 종목은 `-`
  - 누락 시 현재환율로 대체 → **원화 손익은 부정확**(현지통화 수익률은 정확)
- **손절가**: 매입 통화 기준 숫자(예 `96`) 또는 트레일링 규칙(`MA20`, `직전고점-15%`). 미보유 시 `-`
  - `python3 src/tools/cli.py portfolio check` 로 위반/근접 여부를 점검합니다
- **목표가**: 매입 통화 기준 숫자. 미설정 시 `-`
- **진입일**: 최초 진입일 `YYYY-MM-DD`. 미기록 시 `-`
...
|------|------|------|------|----------|--------|--------|--------|
| NVDA | 10 | 120.00 | USD | 1380 | 96 | 180 | 2026-01-15 |
| 005930.KS | 50 | 70000 | KRW | - | MA20 | 90000 | 2026-02-01 |
"""
