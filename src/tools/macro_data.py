...
프록시 주의: 신용은 OAS가 아닌 HYG/LQD 가격비율, 실질금리는 TIP 추세, 10y-2y는 2YY=F
선물 기반이다(모두 method/source 라벨). 의존 심볼 실패 시 해당 신호는 null로 degrade한다.
...
def _fetch_fred_api(series_id, api_key, timeout=10):
    """FRED 공식 API(JSON) — INVEST_FRED_API_KEY 있을 때. 최신 60관측 → 오름차순 [(date, value)]."""
    import json
    url = (f"https://api.stlouisfed.org/fred/series/observations?series_id={series_id}"
           f"&api_key={api_key}&file_type=json&sort_order=desc&limit=60")
...
def _fetch_fred(series_id, timeout=10):
    ...
    if key:
        pts = _fetch_fred_api(series_id, key, timeout)
        if pts:
            return pts
...
def _skeleton(errors=None, note="네트워크 미사용(INVEST_MACRO_LIVE=0) — 모든 값 null"):
    ...
    return {
        ...
        "rating_rule": (
            "risk_score: VIX밴드·HY/IG 3M·구리금 3M·DXY 3M·SPY 이평·QQQ/SPY 3M·net liquidity 4주·VIX급등 "
            "가산 후 [-5,+5] clamp → >=+3 RISK-ON / <=-3 RISK-OFF / else NEUTRAL. "
            "quadrant: 성장축(구리금·곡선·SPY200ma) x 인플레축(10y 3M변화·유가·구리, 상하방 대칭)."
        ),
...
def macro_dashboard(period: str = "1y") -> dict:
    ...
    credit_read = (f"HY/IG 3M {hyg_lqd_3m}% → {spread_signal}"
                   if hyg_lqd_3m is not None else None)
    ...
    if hyg_lqd_3m is not None:
        if hyg_lqd_3m > 1:
            score += 1; drivers.append(f"HYG/LQD 3M +{hyg_lqd_3m}% 타이트닝 → +1")
        elif hyg_lqd_3m < -1:
            score -= 1; drivers.append(f"HYG/LQD 3M {hyg_lqd_3m}% 와이드닝 → -1")
        else:
            drivers.append(f"HYG/LQD 3M {hyg_lqd_3m}% 중립 → 0")
    else:
    ...
    if ((tnx_3m_delta is not None and tnx_3m_delta > 25)
            ...
            or (copper_3m is not None and copper_3m > 10)):
        inflation_axis = 1
    elif ((tnx_3m_delta is not None and tnx_3m_delta < -25)
          or (wti_3m is not None and wti_3m < -10)
          or (copper_3m is not None and copper_3m < -10)):  # 상방과 대칭(OR·구리 포함)
        inflation_axis = -1
    ...
    drivers.append(f"growth_axis={growth_axis}, inflation_axis={inflation_axis} → {quadrant}")
    ...
    elif sp_now is not None and sp_now > 25 and inflation_axis >= 1:
    ...
    return {
        ...
        "rating_rule": (
            "risk_score: VIX밴드(+1/-1/-2)·HY/IG 3M(±1)·구리금 3M(±1)·DXY 3M(±1)·"
            "SPY 50/200이평(±1)·QQQ/SPY 3M(+1)·net liquidity 4주(±1)·VIX 1M급등(-1) "
            "가산 후 [-5,+5] clamp → >=+3 RISK-ON / <=-3 RISK-OFF / else NEUTRAL. "
            "quadrant: 성장축(구리금 3M·bull스티프닝·SPY200ma) x 인플레축(10y 3M변화·유가 3M·구리 3M). "
