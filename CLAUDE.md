...
멀티 에이전트 투자 분석 시스템. Claude Code Agent Teams 기반으로 9+1명의 전문 에이전트가 협업하여 종합 투자 판단을 수행한다.
...
# 보고서 PDF 출력 (report.md/summary.md → 증권사 스타일 PDF, Playwright+pandoc)
python3 src/tools/cli.py report-pdf <TICKER> --meta <path> [--mode deep|brief] [--date YYYY-MM-DD]
python3 src/tools/cli.py report-pdf <TICKER> --meta-stdin --mode deep   # 메타 JSON을 stdin으로
#   → deep=report.md 전체 심층형, brief=summary.md 요약형. 결과: 같은 디렉토리에 report.pdf/summary.pdf
#   → 표지(헤더·Key-Data 사이드바·Risk-Reward 차트)는 메타 JSON, 본문은 report.md 그대로 이식
#   → 표는 Exhibit 번호 자동 부여, 판정 이모지(🟢🔴 등)는 색상 기호(●▲✓)로 치환(이모지폰트 불필요)
#   → 슬래시 커맨드 /invest:report <TICKER> 로도 가능 (메타 JSON 자동 구성)
